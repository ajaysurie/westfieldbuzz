...
- **Firestore security rules block cross-user subcollection writes in admin flows**: When an admin approves a user's suggestion and must write a subcollection doc keyed by `userId` (e.g., `services/{id}/recommendations/{userId}`), the rule `request.auth.uid == userId` blocks the write because admin ≠ suggester. Always add `|| isAdmin()` to any subcollection rule that admin approval flows need to write on behalf of users.
- **`serverTimestamp()` is forbidden inside array values — use `new Date()` instead**: `serverTimestamp()` only works at document field level. Using it inside an array element (e.g., `recentRecommenders: [{ timestamp: serverTimestamp() }]`) throws a Firestore error. Use `new Date()` for timestamps embedded in array objects.
- **Use `writeBatch` whenever a single operation touches multiple documents**: Sequential `await` calls across multiple Firestore docs leave the DB in a partial state if any step fails. Replace with `writeBatch` (create + subcollection set + status update) so all writes succeed or all fail atomically. Especially important for approval flows that create a service, write a recommendation, and update a suggestion status.
